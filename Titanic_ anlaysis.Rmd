...
titanic.train$Age2 <- original.titanic.train$Age

titanic.train %>% filter(is.na(Age2)) -> no_age2
titanic.train %>% filter(!(is.na(Age2))) -> with_age2
median_age2 <- median(with_age2$Age2)
no_age2$Age2 <- median_age2

tt <- rbind(no_age2,with_age2)
tt %>% arrange(PassengerId)
```
...
titanic.train %>% ggplot(., aes(x=Embarked,fill=Survived)) + geom_bar(position = "dodge") +
  labs(title = "Survived vs Embarked")
  

# From observation, Embarked is associated with Survived. However, Embarked is also
# associated with Gender and Pclass.
# People embarked at port Q,S are mostly in Third class.
# People embarked at port C are mostly in First class.

titanic.train %>% ggplot(., aes(x=Embarked,fill=Pclass)) + geom_bar(position = "dodge")
...
pred_log.model <- glm(Survived ~ Sex + SibSp + Age + Pclass,
...
# Decision Tree Accuracy Result: 75.36%
...
# Random forest Accuracy Result: 75.83%
```

```{r}
diff_sum <- 0
for (i in 1:418){
  if (output2$Survived[i] != output3$Survived[i]){
    diff_sum <- diff_sum + 1
  }
  else{
    next
