...
gender_OR <- (gender_table[2,1]/gender_table[1,1])/(gender_table[2,2]/gender_table[1,2])

# The odd ratio is 12.35, the odd of female Survived is 12 times than the odd of male Survived.

# Relative Risk

gender_RR <- (gender_table[2,2]/male_total)/(gender_table[1,2]/female_total)
...
prop.table(table(titanic.train$Pclass,titanic.train$Survived, dnn = c("Pclass","Survived")))

titanic.train %>% group_by(First_class_Survived = Survived) %>% filter(Pclass == "1" ) %>% 
  summarize(count = n(), Proportion = n()/Pclass_count[1,2])

titanic.train %>% group_by(Second_class_Survived = Survived) %>% filter(Pclass == "2" ) %>% 
  summarize(count = n(), Proportion = n()/Pclass_count[2,2])

titanic.train %>% group_by(Third_class_Survived = Survived) %>% filter(Pclass == "3" ) %>% 
  summarize(count = n(), Proportion = n()/Pclass_count[3,2])
...
# chi-square test with fixed gender
...
titanic.train %>% filter(Survived == 1) -> titanic.survive1
titanic.train %>% filter(Survived == 0) -> titanic.survive0
