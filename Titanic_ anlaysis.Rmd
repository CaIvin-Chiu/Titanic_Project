...
output2$Survived <- decision_result
output2$Survived <- ifelse(output2$Survived > 0.5, 1, 0)

write.csv(output, file= "Titanic_decision_tree_prediction.csv", row.names=FALSE)
...
random_forest_model <- randomForest(Survived ~ Sex + Pclass + Fare + Embarked + SibSp,
                                    data = titanic.train, 
...
write.csv(output, file= "Titanic_random_forest_prediction2.csv", row.names=FALSE)
